...
class DomExecutorPlaywright:
    ...
    Uses lazy initialization to avoid crashes during startup.
    """
    ...
    def ensure_login(self, selectors):
        ...
        try:
            ...
            self.page.goto("https://www.bet365.it/#/HO/", wait_until="networkidle", timeout=30000)
            time.sleep(2)
    ...
    def navigate_to_match(self, teams, selectors):
        ...
        Args:
            teams: Team names to search for
        ...
        try:
            ...
            if search_btn.count() > 0:
                search_btn.click()
                time.sleep(0.5)
                self.page.fill(selectors.get("search_input", "input.s-SearchInput"), teams)
                self.page.keyboard.press("Enter")
                time.sleep(1)
            else:
                # Fallback: search for team text directly
                self.page.locator(f"text={teams}").first.click()
                time.sleep(1)
    ...
    def select_market(self, market_name, selectors):
        """
        Select a specific betting market
        
        Args:
            market_name: Name of the market to select
            selectors: Selector configuration
        ...
        try:
            ...
            self.page.locator(f"text={market_name}").first.click()
            time.sleep(0.5)
    ...
    def place_bet(self, selectors):
        ...
        if not self.allow_place:
            self.logger.warning("⚠️ BETS ARE DISABLED (allow_place=False). Skipping real click.")
            return False
        ...
        try:
            ...
            self.page.click(btn_selector, timeout=5000)
