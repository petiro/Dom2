    ...
    def process_signal(self, payload, money_manager):
        ...
        try:
            ...
            if self.executor.check_open_bet():
                ...
                self.bus.emit("BET_FAILED", {"reason": "Bet already open"}) # FIX: emit invece di publish
                return
            ...
            # FIX: Accesso sicuro allo stake (Fallback a 2.0 se il tester non lo imposta)
            ...
            if bet_ok:
                ...
                self.bus.emit("BET_SUCCESS", {"teams": teams, "stake": stake, "odds": odds})
            else:
                self.logger.error("❌ Fallimento piazzamento scommessa")
                self.bus.emit("BET_FAILED", {"reason": "Place bet failed"})

        except Exception as e:
            # 🔴 FIX 1: GARANZIA SBLOCCO LOCK IN CASO DI CRASH
            self.logger.critical(f"🔥 Crash in Execution Engine: {e}")
            self.bus.emit("BET_FAILED", {"reason": str(e)}) # FIX: emit invece di publish